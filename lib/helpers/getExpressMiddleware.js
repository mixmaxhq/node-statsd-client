...
function factory(parentClient) {
  return function(prefix, options) {
    ...
    const timeByUrl = options.timeByUrl || false;
    const notFoundRouteName = options.notFoundRouteName || 'unknown_express_route';
    const onResponseEnd = options.onResponseEnd || undefined;

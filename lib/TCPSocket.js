...
const debug = require('util').debuglog('stats-client');
...
TCPSocket.prototype._enqueue = function _enqueue(data) {
  ...
  if (
    this._buffer.reduce(function(sum, line) {
      return sum + line.length;
    }, 0) > this._maxBufferSize
  ) {
    this._flushBuffer();
  }
...
TCPSocket.prototype._writeToSocket = function _writeToSocket(data, callback) {
  ...
  const message = new Buffer(data.join('\n') + '\n');

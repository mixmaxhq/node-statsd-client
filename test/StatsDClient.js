    ...
    it('.timing("foo", new Date(-20ms)) ~→ "foo:20|ms"', function(done) {
      ...
      setTimeout(function() {
        ...
        setTimeout(function() {
          ...
          const sentMessages = s._packetsReceived;
          assert.lengthOf(sentMessages, 1);
          assert.match(sentMessages[0], /foo:[12]\d\|ms/);
...
describe('StatsDClient / HTTP', function() {
  it('Giving plain host=example.com gives UDP backend', function() {
    assert.instanceOf(new StatsDClient({ host: 'example.com' })._socket, EphemeralSocket);
  });
  ...
  it('Giving host=http://example.com gives HTTP backend', function() {
    assert.instanceOf(new StatsDClient({ host: 'http://example.com' })._socket, HttpSocket);
  });
});

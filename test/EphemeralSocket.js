...
const EphemeralSocket = require('../lib/EphemeralSocket'),
...
describe('EphemeralSocket', function() {
  ...
  it('Respects host-configuration', function(done) {
    const w = new EphemeralSocket({ host: 'some_other_host.sbhr.dk' });
  ...
  it('Convert the port to integer', function() {
    const testSocket = new EphemeralSocket({ port: '1234' });
    assert.strictEqual(testSocket._port, 1234);
  });

  it('Sends data immediately with maxBufferSize = 0', function(done) {
    const withoutBuffer = new EphemeralSocket({ maxBufferSize: 0 }),
      start = Date.now();
  ...
  it("Doesn't send data immediately with maxBufferSize > 0", function(done) {
    const withBuffer = new EphemeralSocket({ socketTimeout: 25 });
